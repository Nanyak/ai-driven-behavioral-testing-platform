 ...
 *
 * The agent may rewrite ARRANGE/ACT (setup, resolve calls, the request itself)
 * but must NEVER touch the ASSERTION/oracle. Without this, an agent told "make it
 * pass" would cheat — weaken `.toBe(200)` to `.toBe(400)`, drop a step, or change
 * the expected outcome — silently disabling the exact regression signal the suite
 * exists to produce.
 *
...
const FORBIDDEN_IF_NEW = [/\btest\.skip\(/, /\btest\.fixme\(/];
