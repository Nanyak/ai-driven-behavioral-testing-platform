 ...
 * consumption.
 *
 * Per-persona PROJECTS (Phase 10): one project per persona folder so the
 * Phase 10 test-runner can scope a run with \`--project <persona>\` and get
 * per-persona pass/fail counts. This config is GENERATED by
 * services/script-generator (run.ts: writeConfigAndFixtures) — edit it there,
 * not by hand; a regeneration clobbers hand edits.
 *
 * Raw Playwright output (JSON + HTML) lands under \`reports/playwright/\`
 * (repo root) per the Phase 10 plan §Location, so reporting (Phase 11) reads
 * a stable, single location. The reporter outputs honor PLAYWRIGHT_JSON_OUTPUT
 * / PLAYWRIGHT_HTML_OUTPUT when the runner sets them, defaulting to the same
 * reports/playwright/ paths.
 */
