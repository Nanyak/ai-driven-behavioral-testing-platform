...
2. **Defensive dedup** (candidates are already deduped in Phase 7, but re-apply): identical step sequences → highest support; common-prefix ≥3 → longest representative; cap 10 per persona. Use the **same canonical flow signature** as Phase 7 (`behavior-engine/src/signature/signature.ts`, ADR 0002) — do not define a second "same flow?" key here.
...
2. **Auth credentials** — the `/auth/*/emailpass[/register]` login endpoints are **not** in the store/admin OAS, so schema synthesis would yield an empty (credential-less) body and the login would `401`. These emit **real, threaded** credentials instead: admin login (`POST /auth/user/emailpass`) reads the same `MEDUSA_ADMIN_EMAIL`/`MEDUSA_ADMIN_PASSWORD` env the shared fixture uses; customer register/login (`POST /auth/customer/emailpass[/register]`) reuse the in-test generated `email`/`password` consts, so a later login matches the registration. The emit setup declares those consts whenever the flow carries any customer-auth step and auto-registers **only** when the flow has no register step of its own (avoiding a duplicate-email double-register).
   ...
   **Customer session = full Medusa v2 handshake, not register alone.** The auto-register setup emits three calls, not one: `POST /auth/customer/emailpass/register` → `POST /store/customers` → `POST /auth/customer/emailpass` (login), and threads the **login** token as `scope.customerToken`. This is a hard requirement, not a nicety: the register token carries an **empty `actor_id`** — it authorizes *creating* the customer entity but does **not** satisfy the `requireCustomerAuth` cart/checkout gate (ADR 0004), so using it directly returns `401` on every gated step (`POST /store/carts*`, `/store/payment-collections*`). Only after `POST /store/customers` links a customer to the auth identity does a fresh login mint a token whose `actor_id` resolves to a real customer — the token the gate accepts. This is **version-sensitive** (Medusa 2.x auth shapes vary by minor); the emitted setup is marked `// VERIFY against live backend` (CLAUDE.md §5). Verified live on the current backend: register-token cart mutation → `401`; post-create login-token cart mutation → `200`.
