...
- **The spec covers errors and happy path via a deterministic overlay (ADR 0004).** Medusa's generator reads routes/validators, not middleware, so middleware-injected responses (the `requireCustomerAuth` `401` gate) and supplemental fragments (ADR 0003 admin reversals) are **not** in the generated base. `build-oas.ts` augments the base spec by injecting those responses for every operation matching a gate rule. Error steps the overlay covers become spec-sourced with provenance instead of observed-only. The merge is deterministic (status-presence check + `schema-merge` union) — **no LLM**.
...
services/golden/                # canonical comparator library; imported by ingestion, vendored into generated-tests by Phase 9
  ...
  openapi/
    build-oas.ts         # ADR 0004: overlay middleware/supplemental responses onto the base spec (deterministic)
...
4. Merge any supplemental fragments (ADR 0003 admin-reversal sequence) the same way.
